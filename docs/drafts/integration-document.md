# Proposal Integration Document

This page publishes the proposal integration layer stored in `proposal/narrative/fire_model_proposal_integration_document.md`.

It is intentionally a drafting artifact rather than polished submission prose. Its job is to hold together the pieces that are easy to lose across separate theory, benchmark, work-plan, and implementation conversations.

!!! note "Why this page matters"
    Use this document when future drafts need to recover the core claim, the geometry-first benchmark logic, the Generative Interface Model framing, or the detect-explain-apply work-plan structure without reassembling them from scattered notes.
